...
- A few gallery examples still read niche upstream sources directly when `investdatar` does not yet expose them as first-class local source adapters. Those cases should be treated as temporary. Treasury local curve data is now available through `investdatar::get_local_treasury_rates()`, while the XML-based Treasury decomposition example remains as a source-specific gallery workflow rather than the default local-data path.
