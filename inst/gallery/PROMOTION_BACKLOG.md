...
| Macro forecast boards | Implemented forecast scripts plus planned `real-data-fred-*-forecast/scenario/probability` and `real-data-yahoo-*-forward-*` gallery scripts | `investlabr::prep_*`, `factor_*`, `sim_*`, `regime_*`, `sense_*`, `brief_*`, and `viz_*` | Forward-looking macro research needs reusable forecast prep, fan charts, probability/regime summaries, scenario comparisons, and briefing visuals while data access stays in `investdatar` |
