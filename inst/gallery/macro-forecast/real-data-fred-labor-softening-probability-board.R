...
wide <- investlabr::prep_series_wide(list(
...
wide[, `:=`(
  unrate_score = investlabr::prep_zscore(UNRATE - shift(UNRATE, 6L)),
  claims_score = investlabr::prep_zscore(log(CCSA) - shift(log(CCSA), 13L)),
  payroll_score = investlabr::prep_zscore(-payroll_3m),
  quits_score = investlabr::prep_zscore(-JTSQUR)
)]
wide[, softening_score := investlabr::factor_bounded_score(rowMeans(.SD, na.rm = TRUE)), .SDcols = c("unrate_score", "claims_score", "payroll_score", "quits_score")]
...
p1 <- ggplot(recent, aes(date, softening_score)) + geom_hline(yintercept = c(40, 70), linetype = "dotted", color = "#777777") + geom_line(color = "#B24C45", linewidth = 0.9) + labs(title = "Labor softening risk score", subtitle = investlabr::brief_score_label(tail(recent$softening_score, 1)), x = NULL, y = "Score (0-100)")
p1 <- mf_theme(p1, "none")
