...
reg <- investlabr::prep_series_wide(list(mf_load_fred("T10YIE", "Inflation expectations", start_date), mf_load_fred("UNRATE", "Unemployment", start_date), mf_load_fred("WRBWFRBL", "Reserves", start_date)))
...
make_asset_fan <- function(symbol) {
  ...
  aligned[, regime := investlabr::prep_fill_forward(regime)]
  changes <- aligned[regime == current_regime & is.finite(ret), ret]
  fan <- investlabr::sim_forward_fan(100, 100 * changes, horizons = c(1L, 5L, 10L, 20L, 60L), seed = nchar(symbol) * 7)
  fan[, asset := symbol]
  fan
}
