...
rsi_tgt <- strategyr::strat_rsi_revert_tgt_pos(
  market_dt,
  n = 14L,
  oversold = 30,
  overbought = 70,
...
bt_res <- build_plot_bt_res(
  ...
  strat_label = sprintf(
    "RSI reversion | n = 14, oversold = 30, overbought = 70, exit = 50, target max lev = %.2f, account lev = %.1f",
    target_max_leverage,
    account_leverage
  )
)
...
cat(
  "This example evaluates an RSI mean-reversion strategy on the S&P 500. The rule uses oversold and overbought RSI thresholds to enter contrarian exposure and an exit threshold to flatten, then compares the path-dependent strategyr backtest against buy-and-hold.\n\n"
)
