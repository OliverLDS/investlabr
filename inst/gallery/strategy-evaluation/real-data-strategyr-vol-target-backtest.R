...
vol_target_tgt <- strategyr::strat_vol_target_tgt_pos(
  market_dt,
  trend_n = 20L,
  rv_n = 20L,
  vol_target = 0.2,
...
bt_res <- build_plot_bt_res(
  equity = run_strategyr_backtest(market_dt, vol_target_tgt, strat_id = 402L),
  datetime = market_dt$datetime,
  asset_name = asset_label,
  strat_label = sprintf(
    "Volatility targeting | trend = 20, rv = 20, vol target = 20%%, target max lev = %.2f, account lev = %.1f",
    target_max_leverage,
    account_leverage
  )
)
...
cat(
  "This example evaluates a volatility-targeting strategy on the S&P 500. The rule scales target exposure from recent realized volatility and a trend filter, capped by the target maximum leverage, then compares the path-dependent strategyr backtest against buy-and-hold.\n\n"
)
