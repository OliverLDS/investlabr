...
source(file.path(gallery_dir, "_shared", "strategyr-backtest.R"))

ticker <- "HYG"
benchmark_ticker <- "LQD"
asset_label <- "iShares High Yield Corporate Bond ETF"
...
backtest_caption <- sprintf(
  "Backtest assumptions: traded asset = %s; spread benchmark = %s; contract size = %.2f; contract step = %.2f; fee ratio = %.4f; funding ratio = %.4f; account leverage = %.1f; target max leverage = %.2f.",
...
spread_tgt <- strategyr::strat_pair_spread_revert_tgt_pos(
  market_dt,
  x_col = "close",
...
bt_res <- investlabr::prep_backtest_result_from_equity(
  equity = run_strategyr_gallery_backtest(market_dt, spread_tgt, 501L, account_leverage, contract_size, contract_step, fee_ratio, funding_ratio),
  datetime = market_dt$datetime,
  asset_name = asset_label,
  strat_label = sprintf(
    "Pair-spread reversion vs LQD | z = 20, entry/exit = 2/0.5, target max lev = %.2f, account lev = %.1f",
    target_max_leverage, account_leverage
  )
)
benchmark_bt_res <- investlabr::prep_backtest_result_from_equity(
...
cat(
  "This example evaluates a pair-spread mean-reversion strategy using HYG as the traded asset and LQD as the spread benchmark. The rule standardizes the HYG-minus-LQD price spread and trades extreme deviations, then compares the path-dependent backtest against buy-and-hold HYG exposure.\n\n"
)
