# Lightweight gallery glue for local data access and execution through
# strategyr. Research result adapters remain package functions in investlabr.
...
load_strategyr_gallery_yahoo <- function(ticker, from_date, to_date) {
  ...
  if (nrow(market_dt) == 0L || !"datetime" %in% names(market_dt)) {
    stop("Local Yahoo OHLC cache is empty or malformed for ticker: ", ticker, call. = FALSE)
  }
  market_dt <- market_dt[
  ...
  if (nrow(market_dt) < 120L) {
    stop("Need at least 120 rows of local OHLC data for this gallery example: ", ticker, call. = FALSE)
  }
  market_dt
}
