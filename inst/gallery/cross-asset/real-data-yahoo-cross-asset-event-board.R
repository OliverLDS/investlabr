...
make_asset_panel <- function(symbols, panel_title) {
  dt_list <- lapply(symbols, function(symbol) {
    ...
    base_ref_date <- max(dt$date[dt$date <= from_date])
  ...
  daily_guides <- data.table::data.table(
    datetime = as.POSIXct(seq(as.Date(min(panel_dt$datetime)), as.Date(max(panel_dt$datetime)), by = "day"), tz = "UTC")
  )
  panel_events <- data.table::copy(events_dt)
...
board <- investlabr::gen_grid_of_plots_with_labels(
  ...
  n_rows = 2,
  n_cols = 2,
  title = "Cross-Asset Performance Around Selected Events",
  bottom = paste0(
